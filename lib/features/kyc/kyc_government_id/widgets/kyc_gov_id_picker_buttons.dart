...
class KycGovIdPickerButtons extends StatelessWidget {
  const KycGovIdPickerButtons({super.key});
      ...
      children: [
        ...
        Expanded(
          child: _PickerButton(
            ...
            label: 'Gallery',
            onTap:
                isUploading ? null : () => _pick(context, fromCamera: false),
          ),
        ),
      ],
    );
  ...
  Future<void> _pick(BuildContext context, {required bool fromCamera}) async {
    try {
      ...
      if (context.mounted) {
        context
            .read<KycGovernmentIdCubit>()
            .onDocumentImageSelected(result?.file);
      }
    } catch (error, stackTrace) {
      logE('Error picking document image: $error', stackTrace: stackTrace);
    }
  }
}
